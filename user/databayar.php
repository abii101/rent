...
<html lang="en">
    ...
    ");
                        ...
                        ?>
                          <tr>
                              ...
                              <td>  <a href="aksi/bayar.php?id_kembali=<?php echo $d['id_kembali']; ?>" class="btn btn-primary w-100">Bayar</a></td>
                          </tr>
                        <?php
                            }
                        ?> 
  ...
  <thead>
    <tr>
      <th>No</th>
      <th>Mobil</th> <!-- Tambah kolom Mobil -->
      <th>Total Bayar</th>
      <th>Status</th>
      <th>Opsi</th>
    </tr>
  ...
  <?php
      ...
      $data = mysqli_query($koneksi, "
          SELECT b.*, t.nik, m.brand, m.type 
        FROM tbl_bayar b
        JOIN tbl_kembali k ON b.id_kembali = k.id_kembali
        JOIN tbl_transaksi t ON k.id_transaksi = t.id_transaksi
        JOIN tbl_mobil m ON t.nopol = m.nopol
        WHERE t.nik = '$nik'  -- Tambahkan filter NIK di sini
      "); // Mengambil data NIK dan mobil melalui join dengan tbl_mobil
      
      while($d = mysqli_fetch_array($data)) {
  ?>
    <tr>
      <td><?php echo $no++ ?></td>
      <td><?php echo $d['brand'] . " " . $d['type']; ?></td> <!-- Menampilkan Mobil -->
      <td><?php echo number_format($d['total_bayar'], 2, ',', '.'); ?></td>
      <td><?php echo $d['status']; ?></td>
      <td>
        <?php if ($d['status'] == 'belum lunas') : ?>
          <a href="" disabled class="btn btn-danger w-100">Lunasi</a>
